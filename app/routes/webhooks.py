...
from app.models.item import Item # To update item status if needed
from app.services.notification_service import notification_service # For outbid notifications
...
def lightspark_webhook():
    ...
    if not webhook_secret:
        logger.error("LIGHTSPARK_WEBHOOK_SECRET is not configured. Cannot verify webhook.")
    ...
    if webhook_secret:
        try:
            ...
            if not hmac.compare_digest(expected_signature, signature):
                logger.warning(f"Lightspark webhook signature mismatch. Expected: {expected_signature}, Got: {signature}")
                abort(403, description="Invalid webhook signature.")
            logger.info("Lightspark webhook signature verified successfully.")
        except Exception as e:
            logger.error(f"Error during Lightspark webhook signature verification: {e}", exc_info=True)
    ...
    try:
        ...
        entity_id = event_data.get("entity_id") # ID of the entity that changed (e.g., payment ID, invoice ID)
        current_status = event_data.get("status") # Current status of the entity
        logger.info(f"Received Lightspark webhook: Type='{event_type}', Entity ID='{entity_id}', Status='{current_status}'")
    ...
    if event_type == "PAYMENT_FINISHED":
        ...
        transaction_type = event_data.get("transaction_type") # e.g., "INCOMING_PAYMENT"
        transaction_status = event_data.get("status") # e.g., "SUCCESS", "FAILED"
        invoice_id = event_data.get("invoice_id") # If this is an incoming payment related to an invoice

        logger.info(f"PAYMENT_FINISHED event received. Transaction ID: {transaction_id}, Type: {transaction_type}, Status: {transaction_status}, Invoice ID: {invoice_id}")
        ...
        if transaction_type == "INCOMING_PAYMENT" and transaction_status == "SUCCESS":
            ...
            try:
                ...
                lightspark_invoice_id_from_webhook = event_data.get("invoice_id") # This is a guess based on general patterns
                
                # If webhook has the `encoded_payment_request` directly:
                encoded_invoice_from_webhook = event_data.get("encoded_payment_request") # Another guess
                ...
                if lightspark_invoice_id_from_webhook:
                    ...
                    logger.warning("No `lightspark_invoice_id` column in Bid model yet, or webhook does not provide it.")
                    logger.warning("Please ensure you store Lightspark's internal invoice ID on your Bid model when creating an invoice.")
                    logger.warning("Falling back to searching by encoded_invoice, which might be less reliable if not directly provided.")

                if not bid_to_update and encoded_invoice_from_webhook:
                    bid_to_update = Bid.query.filter_by(encoded_invoice=encoded_invoice_from_webhook).first()
                ...
                if transaction_type == "INCOMING_PAYMENT" and transaction_status == "SUCCESS":
                    ...
                    if lightspark_invoice_id:
                        bid_to_update = Bid.query.filter_by(encoded_invoice=encoded_invoice_from_webhook).first() # Query by actual invoice
                        ...
                        if bid_to_update:
                            if bid_to_update.status != BidStatus.PAID:
                                ...
                                db.session.commit()
                                logger.info(f"Bid {bid_to_update.id} payment confirmed via webhook.")
                                # Potentially trigger seller payout or mark item as sold
                                # handle_seller_payout.delay(...)
                            else:
                                logger.info(f"Bid {bid_to_update.id} already marked as PAID. Ignoring duplicate webhook.")
                        else:
                            logger.warning(f"No Bid found for Lightspark invoice ID {lightspark_invoice_id}.")
                    else:
                        logger.warning("Incoming payment webhook missing 'invoice_id'. Cannot link to a bid.")

                elif transaction_type == "OUTGOING_PAYMENT" and transaction_status == "SUCCESS":
                    ...
                    lightspark_payment_id = event_data.get("id") # Lightspark's internal payment ID
                    ...
                    logger.info(f"Outgoing payment {lightspark_payment_id} successfully completed.")
                elif transaction_status == "FAILED":
                    # Handle failed payments (incoming or outgoing)
                    logger.error(f"Lightspark transaction {transaction_id} failed. Type: {transaction_type}.")
                    ...
                    if transaction_type == "INCOMING_PAYMENT":
                        ...
                        if lightspark_invoice_id:
                            bid_to_update = Bid.query.filter_by(encoded_invoice=encoded_invoice_from_webhook).first() # Or by lightspark_invoice_id
                            if bid_to_update:
                                bid_to_update.status = BidStatus.FAILED_PAYMENT
                                bid_to_update.updated_at = datetime.utcnow()
                                db.session.commit()
                                logger.warning(f"Bid {bid_to_update.id} payment failed via webhook.")
                                # Notify the bidder about failed payment

                else:
                    logger.info(f"Unhandled TRANSACTION_FINISHED status: {transaction_status} for type: {transaction_type}")
